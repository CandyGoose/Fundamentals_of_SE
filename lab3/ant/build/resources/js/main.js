...
yInput.addEventListener('input', () => {
    ...
    const isValid = /^(?:[0-4]|-[1-2])(?:[.,]\d+)?$/.test(yValue);
    ...
    if (!isNaN(yValue)) {
        ...
        yValid = false;
        messages.innerText = '#{msgs.invalid_y_value}';
        toggleSubmitBtn();
    }
    ...
    for (let mutation of mutationsList) {
        ...
        if (mutation.type === 'childList') {
            ...
            if (!isNaN(rValueFloat)) {
                ...
                rValid = false;
                messages.innerText = '#{msgs.invalid_r_value}';
                toggleSubmitBtn();
            }
            if (rValid) {
                messages.innerText = '';
    ...
    for (let mutation of mutationsList) {
        ...
        if (mutation.type === 'childList') {
            ...
            if (xValid) {
                ...
                messages.innerText = '#{msgs.invalid_x_value}';
                toggleSubmitBtn();
            }
        } else {
            messages.innerText = '#{msgs.invalid_x_value}';
